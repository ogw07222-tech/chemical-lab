...
function vessel(h2Mol: number, hMol: number, hId = "H"): SpeciesState[] {
  return [{ id: "H2", molecule: H2, amountMol: h2Mol, phaseState: phase }, { id: hId, molecule: H_ATOM, amountMol: hMol, phaseState: phase }];
}
...
function reversible(id: string, direction: "FORWARD" | "REVERSE", hId = "H"): ReactionCandidate {
  ...
  return {
    id,
    family: isForward ? "BOND_CLEAVAGE" : "BOND_FORMATION",
    reactantRefs: isForward ? [{ speciesId: "H2", coefficient: 1 }] : [{ speciesId: hId, coefficient: 2 }],
    productGraphs: isForward ? [H_ATOM] : [H2],
    atomMapping: [], bondChanges: [], chargeChanges: [],
    reversible: { pairId: "pair:H2:2H", direction },
    stoichiometry: {
      reactants: isForward ? [{ speciesId: "H2", coefficient: 1 }] : [{ speciesId: hId, coefficient: 2 }],
      products: [{ productIndex: 0, coefficient: isForward ? 2 : 1 }],
    },
...
function branchCandidate(): ReactionCandidate {
  return {
    id: "branch", family: "BOND_CLEAVAGE", reactantRefs: [{ speciesId: "H2", coefficient: 1 }],
    productGraphs: [H_PLUS, H_MINUS], atomMapping: [], bondChanges: [], chargeChanges: [],
    stoichiometry: { reactants: [{ speciesId: "H2", coefficient: 1 }], products: [{ productIndex: 0, coefficient: 1 }, { productIndex: 1, coefficient: 1 }] },
    structuralConfidence: 1, assumptions: ["synthetic branch fixture"], ruleId: "test:branch",
    conservation: { valid: true, delta: { elementDelta: { H: 0 }, atomCountDelta: 0, netChargeDelta: 0 }, reasons: [] },
...
function evaluation(
  ...
  return {
    ...
    feasible: "FEASIBLE", rankScore: 1, status: supportClass === "OPEN" ? "OPEN" : "APPROXIMATED", reasonCodes: [], rank: 1, tie: true,
  };
}

function foundation(candidates: ReactionCandidate[], ranked: RankedReactionEvaluation[]): Phase2ReactionPipelineResult {
  return {
    candidates, evaluations: ranked, ranked, validationCandidates: [],
...
function equilibriumProvider(k = 1): EquilibriumDataProvider {
  return { getEquilibriumConstant: () => ({ equilibriumConstantK: k, referenceTemperatureK: T, standardState: DEFAULT_EQUILIBRIUM_STANDARD_STATE, status: "VERIFIED", confidence: "HIGH", source: { sourceIds: ["phase3b-test"] } }) };
}
...
function run(species: SpeciesState[], candidates: ReactionCandidate[], ranked: RankedReactionEvaluation[], dtS = 1, eq = authority(), timestepId = "step-1") {
  return runPhase3BReactionProgressionFromFoundation(
    { species, elements },
    { environment: { temperatureK: T, pressurePa: P }, dtS, timestepId, startTimeS: 0, thermalState: thermal(), speciesRegistry: createDynamicSpeciesRegistryFromSpecies(elements, species), resolutionOptions: { maxFractionalConsumptionPerStep: 1 }, equilibrium: eq },
...
describe("Phase 3B reversible pair arbitration", () => {
  ...
  it("C near equilibrium produces zero coarse net extent without state snap", () => {
  ...
  it("D repeated approach shrinks extent", () => {
  ...
  it("E enforces PR51 anti-crossing extent", () => {
  ...
  it("F avoids persistent ping-pong", () => {
    let current = vessel(0.9, 0.1);
  ...
  it("G is candidate-order invariant", () => {
  ...
  it("H never double-commits determinate forward/reverse pair", () => {
    const result = run(vessel(0.9, 0.1), [F(), R()], evals(10));
    expect(result.resolution.selected).toHaveLength(1);
    expect(result.thermal.committedContributionCount).toBe(1);
  });

  it("I surviving channel uses ordinary shared-reactant competition", () => {
  ...
  it("J INDETERMINATE/OPEN abstains", () => {
    const result = arbitrateReversiblePairs({ species: vessel(0.9, 0.1), candidates: [F(), R()], rankedEvaluations: evals(), temperatureK: T, pressurePa: P, authority: authority({}) });
    expect(result.pairs[0]!.equilibriumDirection).toBe("INDETERMINATE");
    expect(result.pairs[0]!.equilibriumScientificStatus).toBe("OPEN");
    expect(result.candidateExtentControls).toEqual({});
  });

  it("K does not fabricate extent from OPEN kinetics", () => {
  ...
  it("L preserves heat sign and aggregate-once semantics", () => {
  ...
  it("M generated species participates as a reverse reactant on the next timestep", () => {
    ...
    const first = runPhase3BReactionProgressionFromFoundation(
      ...
      { environment: { temperatureK: T, pressurePa: P }, dtS: 1, timestepId: "gen", startTimeS: 0, thermalState: thermal(), speciesRegistry: createDynamicSpeciesRegistryFromSpecies(elements, initial), resolutionOptions: { maxFractionalConsumptionPerStep: 1 }, equilibrium: authority() },
    ...
    const second = runPhase3BReactionProgressionFromFoundation(
      ...
      { environment: { temperatureK: T, pressurePa: P }, dtS: 1, timestepId: "next", startTimeS: 1, thermalState: first.nextState.thermalState, speciesRegistry: first.nextState.speciesRegistry!, resolutionOptions: { maxFractionalConsumptionPerStep: 1 }, equilibrium: authority() },
      foundation([F(generated.id), R(generated.id)], [evaluation("forward", "FORWARD", 0.1), evaluation("reverse", "REVERSE", 0.1)]),
    );
    expect(second.arbitration.pairs[0]!.equilibriumDirection).toBe("INDETERMINATE");
    expect(second.resolution.selected.some((x) => x.candidateId === "reverse")).toBe(true);
  });

  it("N same-step generated reverse reactants remain ineligible", () => {
    const initial: SpeciesState[] = [{ id: "H2", molecule: H2, amountMol: 1, phaseState: phase }];
    const futureId = `generated:${H_ATOM.canonicalKey}`;
    const result = runPhase3BReactionProgressionFromFoundation(
      { species: initial, elements },
      { environment: { temperatureK: T, pressurePa: P }, dtS: 1, timestepId: "no-cascade", startTimeS: 0, thermalState: thermal(), speciesRegistry: createDynamicSpeciesRegistryFromSpecies(elements, initial), resolutionOptions: { maxFractionalConsumptionPerStep: 1 }, equilibrium: authority() },
  ...
  it("O deterministic replay preserves arbitration, extent, state, events, heat", () => {
  ...
  it("timestep sensitivity keeps dt and dt/2 states finite and non-negative", () => {
    const initial = vessel(0.9, 0.1);
