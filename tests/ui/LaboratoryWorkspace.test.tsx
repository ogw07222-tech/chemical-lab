...
describe('LaboratoryWorkspace', () => {
  ...
  it('normal inventory exposes starters but not all supported species', () => { renderLab(); const inventory = screen.getByText('Materials').closest('aside'); expect(inventory).not.toBeNull(); expect(within(inventory!).getByText('Hydrogen')).toBeInTheDocument(); expect(within(inventory!).queryByText('Methane')).not.toBeInTheDocument(); });
  ...
  it('adds a finite amount through the provider', () => { renderLab(); fireEvent.click(screen.getByRole('button',{name:/Add H₂ to vessel/i})); fireEvent.click(screen.getByRole('button',{name:'Timeline'})); expect(screen.getByText(/Added 0.250 mol H₂/)).toBeInTheDocument(); });
  it('uses heater power and thermostat commands rather than direct temperature mutation', () => { renderLab(); fireEvent.change(screen.getByLabelText('Heater power'),{target:{value:'250'}}); expect(screen.getByText('250 W')).toBeInTheDocument(); fireEvent.click(screen.getByLabelText('Thermostat')); fireEvent.click(screen.getByRole('button',{name:'Timeline'})); expect(screen.getByText(/Thermostat enabled/)).toBeInTheDocument(); });
  it('confirms the mock unknown before discovery unlock feedback', () => { renderLab(); expect(screen.queryByText('Water')).not.toBeInTheDocument(); fireEvent.click(screen.getByRole('button',{name:'Analyze'})); fireEvent.click(screen.getByRole('button',{name:'Timeline'})); expect(screen.getByText(/Identity confirmed: Water/)).toBeInTheDocument(); expect(screen.getByText('Water')).toBeInTheDocument(); });
